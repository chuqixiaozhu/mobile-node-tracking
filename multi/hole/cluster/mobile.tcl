...
set opt(radius_cluster) [expr \
       $opt(target_speed_max) * $opt(target_radius_time)]; # Cluster's radius
...
set fixed_ratio \
    [expr pow($opt(hole_length), 2) / pow($opt(x), 2) * $opt(nfnode)]
set opt(nfnode) [expr $opt(nfnode) - $fixed_ratio * $opt(hole_number)]
set opt(nn) \
...
# Fixed nodes tracing contribution
        ...
        set f_node_cluster_act_num 0; # add 150324
		...
		# add 150322-17:29
                ...
                # /add
                incr f_node_cluster_act_num; # add 150324
                $fnode($i) color "green"
                set f_local_proba \
                    [local_probability $dist $opt(dist_threshold_f)]
                set sys_proba_tmp \
                ...
                # modified 150324
                if {$f_node_cluster_act_num == $opt(cluster_node_max)} {
                    break
                }
            }
            set sys_proba [expr 1.0 - $sys_proba_tmp]
...
# For every fixed node, choose a taraget
    for {set i 0} {$i < $opt(nfnode)} {incr i} {
        ...
        for {set j 0} {$j < $opt(ntarget)} {incr j} {
            ...
            if {$dist > $opt(radius_cluster)} { # modified 150324
                continue
            }
            if {!$switch_on} {
                set switch_on 1

...
for i in range(count):
    for target_num in range(2, 21, 2):
        subprocess.call(['ns', 'mobile.tcl', str(target_num), result_file])

# Process the results
rf = open(result_file, 'r')
vt_file = open('target_num-m-fl_vs_vt', 'w')
ec_file = open('target_num-m-fl_vs_ec', 'w')
vt = dict()
ec = dict()
for line in rf:
    results = line.split()

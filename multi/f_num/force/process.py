...
for i in range(count):
    for num_fnode in range(40, 201, 20):
        subprocess.call(['ns', 'mobile.tcl', str(num_fnode), result_file])

# Process the results
rf = open(result_file, 'r')
vt_file = open('f_num-m-force_vs_vt', 'w')
ec_file = open('f_num-m-force_vs_ec', 'w')
vt = dict()
ec = dict()
for line in rf:
    results = line.split()

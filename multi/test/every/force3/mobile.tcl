...
set opt(weight_GT) 300;                # Weight of attracting force from target
set opt(weight_GM) 100;     # Weight of repulsive force from other mobile nodes
...
if {0 < $argc} {
    ...
    set opt(result_file) [lindex $argv 0]
...
proc mobile_node_action {time_stamp} {
    global opt mnode target lag to_target avg_mnode
    for {set i 0} {$i < $opt(ntarget)} {incr i} {
...
# Computing the force from this target($j)
    ...
    for {set i 0} {$i < $opt(ntarget)} {incr i} {
        incr avg_mnode($i) [llength $moving_mnode_index($i)]
    }
...
# Computing the consumption of Fiexed nodes, as well as valid surveillance time
    #if {0 < $num_mobile} {
    #    fixed_node_action moving_mnode_index $num_mobile $time_stamp
    #} else {
    #    fixed_node_action "" 0 $time_stamp
    #}
    ...
    #    } else {
    #        fixed_node_action $i "" 0 $time_stamp
    #    }
    #}
...
proc output_file {} {
    ...
    for {set i 0} {$i < $opt(ntarget)} {incr i} {
        ...
        puts -nonewline $result_file [expr 1.0 * $avg_mnode($i) / $opt(stop)]
    }
    puts $result_file ""
    flush $result_file
    close $result_file

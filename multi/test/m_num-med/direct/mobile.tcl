...
set opt(MED_EMT) 0;           # Median Effective Monitoring Time of targets
...
# Calculate Median Effective Monitoring Time of targets
proc median_emt {} {
    global EMT opt
    set emt_list ""
    for {set i 0} {$i < $opt(ntarget)} {incr i} {
        lappend emt_list $EMT($i)
    }
    set emt_list [lsort -real $emt_list]
    set opt(MED_EMT) [lindex $emt_list [expr $opt(ntarget) / 2]]
}
...
proc finish {} {
    ...
    getting_results
    puts "median_effective_monitoring_time = $opt(MED_EMT)"

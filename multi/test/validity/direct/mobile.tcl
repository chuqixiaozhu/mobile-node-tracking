...
set opt(stop)   3                        ;# time of simulation end
set opt(nfnode) 10                        ;# number of fixed nodes
...
set opt(dist_threshold_f) 3             ;# Distance threshold of Fixed nodes
...
set opt(dist_limit) 7;        # Maximum distance from target to active nodes
...
set opt(weight_GM) 400;     # Weight of repulsive force from other mobile nodes
...
proc local_probability {dist dist_threshold} {
    ...
    puts "@382 source_intensity: $source_intensity"
    set member \
        [expr $opt(sen_intensity_threshold) \
             - $source_intensity - $opt(noise_avg)]
    puts "@386 member: $member"
    ...
    puts "@414 local_proba: $local_proba"
    return $local_proba
}
...
# Fixed nodes tracing contribution
        ...
        puts "num_sensing_fnode: $num_sensing_fnode"
        if {$num_sensing_fnode > 0} {
            ...
            foreach i_d $fnode_list {
                ...
                set f_local_proba \
                    [local_probability $dist $opt(dist_threshold_f)]
                puts "f_local_proba: $f_local_proba"
                set sys_proba_tmp \
                    [expr $sys_proba_tmp * (1.0 - $f_local_proba)]
                puts "sys_proba_tmp: $sys_proba_tmp"
                set sys_proba [expr 1.0 - $sys_proba_tmp]
                puts "fixed sys_proba: $sys_proba"
                if {$opt(sys_proba_threshold) <= $sys_proba} {
                    incr EMT($target_index) $opt(time_click)
                    puts "++++++++++++++++++++++++++++++++++++++"
                    break

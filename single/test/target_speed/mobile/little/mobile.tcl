...
set opt(x)      10                        ;# X dimension of topography
set opt(y)      10                        ;# Y dimension of topography
set opt(stop)   100                        ;# time of simulation end
set opt(nfnode) 3                        ;# number of fixed nodes
set opt(nmnode) 2                         ;# number of mobile nodes
set opt(node_size) 1                       ;# Size of nodes
set opt(target_size) 2                     ;# Size of the target
set opt(radius_m) 7                       ;# Sensing Radius of Mobile nodes
...
set opt(dist_limit) 5;        # Maximum distance from target to active nodes
...
#set opt(target_speed_min) [expr $opt(target_speed_max) - 2.3]
#if {$opt(target_speed_min) < 0} {
#    set opt(target_speed_min) 0.7
#}
set opt(nn) \
...
proc tracking_target_computing {m_m_index_ s_f_index_ time_stamp} {
    ...
    puts "Let's compute now. Mobile first."
...
# Mobile nodes tracking contribution
        ...
        if {0 < $num_moving_mnode} {
            foreach i_d $moving_mnode_index($target_index) {
                set i [lindex $i_d 0]
                puts "Mobile node $i is TRACKING"
            ...
            if {$opt(sys_proba_threshold) <= $sys_proba} {
                ...
                puts "Mobile BREAK. sys_proba is enough. EMT = $EMT($target_index)"
                continue
            }
        }
...
# Fixed nodes tracing contribution
		...
		# add 150322-17:29
                if {$lag([$fnode($i) id])  <= $opt(lag_time)} {
                    puts "Fixed node $i lag = $lag([$fnode($i) id]), NOT enough"
                    continue
                }
                puts "Fixed node $i lag = $lag([$fnode($i) id]), OK and DETECTING"
                # /add
                $fnode($i) color "green"
                set f_local_proba \
                    [local_probability $dist $opt(dist_threshold_f)]
                ...
                if {$opt(sys_proba_threshold) <= $sys_proba} {
                    incr EMT($target_index) $opt(time_click)
                    puts "Fixed BREAK. sys_proba is enough. EMT = $EMT($target_index)"
                    break
                }
            }
        }
    ...
    puts "active_fnode_num = $active_fnode_num, energy_consumption = $opt(energy_consumption)"
}
...
# For every fixed node, choose a taraget
    for {set i 0} {$i < $opt(nfnode)} {incr i} {
        ...
        for {set j 0} {$j < $opt(ntarget)} {incr j} {
            ...
            lappend sensing_fnode_index($j) [list $i $dist]
            puts "Fixed node $i JOINED"
        }
    }
    ...
    for {set i 0} {$i < $opt(nmnode)} {incr i} {
        ...
        for {set j 0} {$j < $opt(ntarget)} {incr j} {
            ...
            if {$lag([$mnode($i) id]) <= $opt(lag_time)} {
                puts "Mobile node $i lag = $lag([$mnode($i) id]), NOT enough"
                break
            } else {
                puts "Mobile node $i lag = $lag([$mnode($i) id]), OK and JOINED"
                lappend moving_mnode_index($j) [list $i $dist]
                set_destination $mnode($i) $target($j) $time_stamp
            }
        }
    }

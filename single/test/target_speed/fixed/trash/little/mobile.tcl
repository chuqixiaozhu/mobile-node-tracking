...
for {set i 0} {$i < $opt(ntarget)} {incr i} {
    ...
    $ns at 0 "$target($i) shape \"hexagon\""
    set EMT($i) 0
}
...
# Fixed nodes tracing contribution
		...
		# add 150322-17:29
                ...
                puts "fixed node $i lag = $lag([$fnode($i) id]), OK."
                # /add
                $fnode($i) color "green"
                set f_local_proba \
                    [local_probability $dist $opt(dist_threshold_f)]
                ...
                if {$opt(sys_proba_threshold) <= $sys_proba} {
                    incr EMT($target_index) $opt(time_click)
                    puts "BREAK. sys_proba is enough. EMT($target_index) = $EMT($target_index)"
                    break
                }
            }
        }

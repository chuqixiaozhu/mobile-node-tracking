...
set opt(location_offset) 2;   # Manual perturbance for target's position
set opt(Pi) [expr acos(-1)];  # Pi
...
# Give a manual perturbance to the Target's position
proc wrong_location {t_x_ t_y_ time_stamp} {
    global opt
    upvar 1 $t_x_ t_x
    upvar 1 $t_y_ t_y
    set rng_uni [new RNG]
    $rng_uni seed 0
    set delta $opt(location_offset)
    set theta [$rng_uni uniform 0 $opt(Pi)]
    set dest_x [expr $t_x + $delta * cos($theta)]
    set dest_y [expr $t_y + $delta * sin($theta)]
    while {$dest_x < 0 || $dest_x > $opt(x) \
        || $dest_y < 0 || $dest_y > $opt(y)} {
        set theta [$rng_uni uniform 0 $opt(Pi)]
        set dest_x [expr $t_x + $delta * cos($theta)]
        set dest_y [expr $t_y + $delta * sin($theta)]
    }
...
proc set_destination {node target itime} {
    ...
    #puts "At $itime: ($target_x, $target_y)"; # test
    wrong_location target_x target_y $itime
    set dest_x $target_x
    set dest_y $target_y
    #puts "\t($dest_x, $dest_y)"; # test

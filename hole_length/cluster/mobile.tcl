...
set opt(hole_length) 90;                    # Length of the hole edge
...
if {0 < $argc} {
    ...
    set opt(hole_length) [lindex $argv 2]
...
proc create_holes {x_ y_} {
    ...
    set margin_x [expr $opt(x)/2.0 - $opt(hole_length)/2.0]
    set margin_y [expr $opt(y)/2.0 - $opt(hole_length)/2.0]
    while {$margin_x <= $x && $x <= $opt(x) - $margin_x && \
           $margin_y <= $y && $y <= $opt(y) - $margin_y} {
        set x [$rd_x value]
        set y [$rd_y value]
    }
...
# If node should be in the cluster
proc be_in_cluster {node target radius itime} {
    return [be_sensed $node $target $radius $itime]
}
